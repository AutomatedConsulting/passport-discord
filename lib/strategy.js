...
Strategy.prototype.userProfile = function(accessToken, done) {
    ...
    this._oauth2.get('https://discord.com/api/users/@me', accessToken, function(err, body, res) {
        ...
        self.checkScope('connections', accessToken, function(errx, connections) {
            if (errx) {
              errx.passportRawProfile = { type: this.name, data: profile };
              return done(errx);
            }
            ...
            self.checkScope('guilds', accessToken, function(erry, guilds) {
                if (erry) {
                  erry.passportRawProfile = { type: this.name, data: profile };
                  return done(erry);
                }
...
Strategy.prototype.authorizationParams = function(options) {
    ...
    if (typeof options.disable_guild_select !== 'undefined') {
      params.disable_guild_select = options.disable_guild_select
    }
    if (typeof options.guild_id !== 'undefined') {
